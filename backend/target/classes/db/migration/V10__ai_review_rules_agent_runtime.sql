...
CREATE TABLE IF NOT EXISTS ai_review_rules (
  ...
  pass_threshold DECIMAL(10, 4) NOT NULL DEFAULT 4.0000,
  need_human_threshold DECIMAL(10, 4) NOT NULL DEFAULT 3.0000,
...
SELECT
  '默认质量预审规则',
  '你是 LabelHub 的 AI 预审员。请根据题目原始数据 {{rawPayload}}、标注答案 {{answer}}、表单 schema {{schema}} 和规则 {{rule}} 进行质量审核。按维度评分，给出总分、风险标签、证据和 PASS / NEED_HUMAN_REVIEW / REJECT 判定。',
  CAST('[{"key":"relevance","label":"相关性","weight":0.35,"maxScore":5},{"key":"accuracy","label":"准确性","weight":0.35,"maxScore":5},{"key":"format_compliance","label":"格式合规","weight":0.2,"maxScore":5},{"key":"safety","label":"安全风险","weight":0.1,"maxScore":5}]' AS JSON),
  4.0000,
  3.0000,
  2,
  30,
