    ...
    },
    {
      ...
      render: (_, rule) => (
        <Space direction="vertical" size={2}>
          ...
          <Text style={{ fontSize: 12 }}>
            REJECT &lt; <Text strong>{rule.needHumanThreshold}</Text>
          </Text>
        </Space>
      ),
    },
    ...
    >
      <Form form={form} layout="vertical">
        ...
        <Form.Item
          name="promptTemplate"
          label="Prompt 模板"
          extra="评分对象应是“标注员提交的标注答案质量”。支持占位符 {{rawPayload}}、{{answer}}、{{schema}}、{{rule}},调用 LLM 前会被填充。"
          rules={[{ required: true, message: '请输入 Prompt 模板' }]}
        >
          <Input.TextArea rows={10} placeholder="你是严格的标注质检员……" />
        </Form.Item>
        ...
        <Row gutter={12}>
          <Col span={12}>
            <Form.Item
              name="passThreshold"
              label="PASS 阈值"
              extra="综合分 ≥ 此值时,LLM 输出 decision 应为 PASS"
              rules={[{ required: true }]}
            >
          ...
          <Col span={12}>
            <Form.Item
              ...
              label="NEED_HUMAN 阈值"
              extra="综合分 < PASS 且 ≥ 此值时应为 NEED_HUMAN_REVIEW;< 此值时应为 REJECT"
              rules={[{ required: true }]}
            >
              <InputNumber step={1} min={0} max={100} style={{ width: '100%' }} />
            </Form.Item>
